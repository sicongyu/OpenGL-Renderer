...
int main(void)
{
	...
	glfwSetWindowTitle(window,"Tutorial 02");
	...
	GLuint programID = LoadShaders("shaders/Transform.vertexshader", 
							"shaders/Transform.fragmentshader");
							
	// Get a handle for our "MVP" uniform
	//GLuint M_ID = glGetUniformLocation(programID, "M");
	GLuint V_ID = glGetUniformLocation(programID, "V");
	GLuint P_ID = glGetUniformLocation(programID, "P");
	GLuint theta_ID = glGetUniformLocation(programID, "theta");
	...
	cube_mesh.setAtrribute(Uv, (void *)g_uv_buffer_data, sizeof(g_uv_buffer_data));
	cube_mesh.vertex_size = sizeof(g_vertex_buffer_data) / sizeof(float) / 3;
	cube_mesh.face_size = cube_mesh.vertex_size / 3;
	cube_mesh.LoadTexture(Diffuse,"models/uvtemplate.DDS");
	...
	do{
		...
		//glm::mat4 ModelMatrix = glm::rotate(glm::mat4(1.0),angle,glm::vec3(0,1,0));

		// Send our transformation to the currently bound shader, 
		glUniformMatrix4fv(V_ID, 1, GL_FALSE, &ViewMatrix[0][0]);
		glUniform3fv( theta_ID, 1, &theta[0] );
		glUniformMatrix4fv(P_ID, 1, GL_FALSE, &ProjectionMatrix[0][0]);
